...
class Decoder(nn.Module):
    ...
    def forward(self, x0, x1, x2, x3, x4, return_features=False):
        ...
        feat16 = self.decoder_layer0(out)  # 1, 16, 224, 224
        out = self.output_layer0(feat16)   # 1, C, 224, 224

        if return_features:
            return out, out1, out2, out3, out4, feat16
        return out, out1, out2, out3, out4

class ResLSTMUNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, pretrained=False, deep_sup=False,
                 multiscale_att=False, dropout_p=0.0, predict_uncertainty=False):
    ...
    def forward(self, x_serial):
        ...
        for t in range(temporal):
            ...
            if self.predict_uncertainty:
                out, out1, out2, out3, out4, feat16 = self.decoder(
                    x0, x1, x2, x3, x4, return_features=True)
                unc_serial.append(self.uncertainty_head(feat16))
            else:
        ...
        if self.deep_sup:
            if self.predict_uncertainty:
                return out_serial, out1_serial, out2_serial, out3_serial, out4_serial, unc_serial
            return out_serial, out1_serial, out2_serial, out3_serial, out4_serial
        else:
            if self.predict_uncertainty:
                return out_serial, unc_serial
            return out_serial